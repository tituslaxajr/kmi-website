...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  return { title: update?.title || "Field update", description: update?.summary };
}
...
export default async function UpdateDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <>
      <Breadcrumbs items={[{ href: "/field-updates", label: "Field updates" }, { label: update.title }]} />
      <article className="article-layout shell">
        <header className="article-header"><p className="eyebrow">{program?.title} · {formatDate(update.date)}</p><h1>{update.title}</h1><p className="article-deck">{update.summary}</p><p className="church-line large">A field update from <Link href={`/partner-churches/${church?.slug}`}>{church?.name}</Link>.</p></header>
      ...
      <section className="article-cta"><div className="shell narrow"><p className="eyebrow light">Respond to this update</p><h2>Pray first. Then follow the next milestone.</h2><div className="button-row"><Link className="button button-light" href="/active-needs">See related needs</Link><Link className="button button-ghost-light" href="/field-updates">More field updates</Link></div></div></section>
    </>
  );
}
